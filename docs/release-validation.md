...
| 实际页面 | 研究入口、完成报告与管理员运行列表可打开，截图位于 `docs/images` |
...
包含当前 Go/Python/Vue 业务代码、回归测试、契约、规格、中文文档和真实 fixture 截图。保留本地原始提交历史。

不发布 `.env`、API 凭据、数据库、依赖缓存、运行日志、临时审查目录或私有 GoSaaS 源码。本地原文件保留。原创代码的开源许可证尚未指定，详情见 [许可说明](../LICENSE.md)。
